...
| Implicit index access on initializers | ✅ Supported | Validated by `Lang13ImplicitIndexInitializerTests.cs::ImplicitIndexInInitializer` via `new Buffer { Items = { [^1] = 0 } }`. Composes the Phase F6 `BoundFromEndIndexExpression` lowering with a nested object-initializer position; the standalone `arr[^1] = 0` form is already covered by `Lang8IndexRangeTests.cs`. |
