            ...
            bool isMcs)
        {
            ...
            catch
            {
                if (isMcs
                    && !Path.GetFileNameWithoutExtension(manifestFileName).EndsWith("Mcs"))
                {
                    string fileName = Path.GetFileNameWithoutExtension(manifestFileName.Substring("Cs2JsC.Clr.Test.".Length)).Replace('.', '\\');
                    ...
                    using (XmlWriter writer = XmlWriter.Create(
                        @"D:\Users\Gautam\Documents\Visual Studio 2010\Projects\Cs2JsC_Mono\Cs2JsC.CLR.Test\"
                        + fileName + "Mcs.xml",
                        settings))
                    {
