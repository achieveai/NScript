...
| Feature | Status | Notes |
|---|---|---|
| Collection expressions (`[1, 2, 3]`) | ⚠️ Partial | Phase E supports `T[]` targets only — `BoundCollectionExpression` lowers to `InlineArrayInitialization`. Validated in `Lang12CollectionExpressionTests.cs`. Interface targets (`IEnumerable<T>` / `IList<T>` / `IReadOnlyList<T>` / `ICollection<T>` / `IReadOnlyCollection<T>`) require Roslyn to resolve `System.Collections.Generic.List<T>..ctor()` as a well-known member during binding, which NScript's `mscorlib` facade does not currently satisfy — they are deferred to **Phase F** alongside `List<T>` targets, `[CollectionBuilder]` types and spread elements. `Span<T>` / `ReadOnlySpan<T>` remain Non-Goals. |
| Spread `..` element | ❌ Deferred | Stage 1 surfaces a clear `NotSupportedException` when a `BoundCollectionExpressionSpreadElement` is encountered. **Phase F.** |
...
| `params` collections | ❌ Deferred | `params T[]` continues to work via the existing path. `params IEnumerable<T>`/`params IList<T>`/etc. share the interface-target collection-expression lowering and inherit the same Phase F gating (Roslyn well-known `List<T>..ctor()` not satisfied by `mscorlib` facade). `params Span<T>` / `params ReadOnlySpan<T>` are Non-Goals. |
