...
keyframesBlock
    : keyframeSelectors LBRACE (decls=declarationSet)? RBRACE
        -> ^(KEYFRAME keyframeSelectors $decls?)
    ;
...
pseudo
    : COLON IDENT LPAREN pseudoFuncArgs? RPAREN
		-> ^(PSEUDO_FUNC IDENT pseudoFuncArgs?)
	| COLON IDENT
		-> ^(PSEUDO IDENT)
    ;
...
term
    ...
    | IDENT LPAREN funcArgs RPAREN
        -> ^(FUNCTION IDENT funcArgs)
    | URI -> ^(URL_VAL URI)
