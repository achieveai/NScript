...
| Target-typed `new()` | ✅ Supported | `ConversionKind.ObjectCreation` is now in `VisitConversion`'s pass-through cluster (alongside `Deconstruction`/`SwitchExpression`/`CollectionExpression`), forwarding to the inner `BoundObjectCreationExpression` so the ordinary constructor pipeline emits the constructed instance. Validated in `Lang9Features.cs::TargetTypedNew*` (parameterless, with-initializer, with-args, as-return) and runtime-gated by `Lang9FeatureExecutionTests` (V8 execution snapshot — asserts field/ctor values across all four shapes). |
...
| Pattern matching — type pattern (without var) | ✅ Supported | Declaration patterns in `is` form (`d is Derived`) and inside `switch`-expression arms (`o switch { string s => ..., int i => ..., }`) both lower through `PatternMatcher.LowerDeclarationPattern`. The arm-local's `ScopeDesignatorOpt` is the arm syntax itself, which is not a block scope; `BoundAstToAstBase.GetLocalVariable` walks the syntactic chain up to the nearest enclosing scope on `scopeBlockStack`, hoisting the local to its natural JS binding site for the inline `(local = AsType(T, scrutinee)) != null` test. Runtime-gated by `Lang9PatternExecutionTests` (V8 execution snapshot — covers `is`-form against a base/derived hierarchy and switch-arm declaration-pattern dispatch via `TestSwitchDeclarationArm`). |
...
| Module initializers | ✅ Supported | `[ModuleInitializer]` BCL facade lives in NScript's `mscorlib` (`System.Runtime.CompilerServices.ModuleInitializerAttribute`); `Builder.cs` collects every static / parameterless / void method carrying the attribute in stable declaration order and emits invocations directly before the entry-point call. Validated by `Lang9FeatureExecutionTests` (V8 execution snapshot — asserts initializer ran before `Main` and that ordering across two tracker types is deterministic). |
...
| Caller argument expression | ✅ Supported | Validated in `Lang10Features.cs::CallerArgumentExpression`. The BCL facade `System.Runtime.CompilerServices.CallerArgumentExpressionAttribute` lives in NScript's `mscorlib` so Roslyn binds the attribute; Roslyn folds the captured argument expression to a literal `string` default at every call site at bind time, so the bound tree at the callee is identical to an ordinary optional `string` parameter and the wire-through is transparent for Stage 1 / Stage 2. No runtime check is emitted (Roslyn already folded the value); this matches NScript's "compile-time strict, runtime permissive" precedent. |
...
| Generic attributes | ✅ Supported | Validated in `Lang11GenericAttributeTests.cs` — defines `MarkerAttribute<T> : Attribute` and exercises generic-attribute applications with concrete reference (`[Marker<string>]`), value (`[Marker<int>]`), user-defined (`[Marker<TargetedClass>]`), and BCL primitive (`[Marker<long>]`) type arguments across class, method, and property targets, plus both parameterised and parameterless ctor overloads. Empirical-probe audit (see issue #72): `SymbolSerializer` already emits `GenericInstanceTypeSer` for any `NamedTypeSymbol` with `Arity > 0` (`SymbolSerializer.cs:257-278`), and Cecil reads `CustomAttribute.AttributeType` as `GenericInstanceType` for the consumer side — no bespoke serializer work was needed. Sibling-file layout follows the `Lang9RecordTests.cs` precedent (file is NOT in `TestResources.cs`'s explicit Roslyn-driven list; MSBuild's `Sources/Framework` glob exercises it end-to-end). |
