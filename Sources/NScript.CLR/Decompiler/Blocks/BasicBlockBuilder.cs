namespace NScript.CLR.Decompiler.Blocks
{
    ...
    internal class BasicBlockBuilder
    {
        ...
        /// <summary>
        /// Gets the first index of the dependent.
        /// </summary>
        /// <param name="block">The block.</param>
        ...
        /// <summary>
        /// Builds the basic blocks.
        /// </summary>
        /// <param name="block">The block.</param>
        /// <param name="consumerBlock">The consumer block.</param>
        ...
        /// <summary>
        /// Merges the into same level blocks.
        /// </summary>
        /// <param name="block">The block.</param>
        /// <param name="containsConsumerBlock">if set to <c>true</c> [contains consumer block].</param>
        /// <param name="boundaries">The boundaries.</param>
        /// <param name="isMergedBlock">The is merged block.</param>
            ...
            IList<bool> isMergedBlock)
        {
            ...
            {
                IlInstruction instruction = block.Children[boundaries[iBoundary] - 1].GetInstructionAt(0);

                if (instruction.GetPushes() > 0)
                {
                    minStackLevel = Math.Min(minStackLevel, instruction.StackLevelAfter);
                    maxStackLevel = Math.Max(maxStackLevel, instruction.StackLevelAfter);
                }
            }
            ...
            if (containsConsumerBlock
                && lastStackBuilderBlock > 1)
            {
                // In this case out lastStackBuilderBlock may still contain DupCopy. Let's make sure that
                // we don't make it a seperate block.

                // Check if this block really puts anything on the stackLevel.
                //
                IlInstruction blockBeginInstruction = block.Children[boundaries[lastStackBuilderBlock - 1]].GetInstructionAt(0);
                IlInstruction blockEndInstruction = block.Children[boundaries[lastStackBuilderBlock] - 1].GetInstructionAt(0);

                // Even though we have splitted Dup in 2 instruction, dup and dupCopy, we still want
                // them to be part of same SerialBlock. To make this happen if we hit dupCopy
                // as beginInstruction, move it to dup block's being instruction and skip
                // analysis of dupBlock.
                //
                // We only do this if DupCopy is not being consumed at this level.
                if (blockBeginInstruction.OpCode == OpCode.DupCopy
                    && blockBeginInstruction.StackLevelBefore >= minStackLevel
                    && blockEndInstruction.StackLevelAfter == blockBeginInstruction.StackLevelBefore)
                {
                    // mark Dup block as merged block.
                    isMergedBlock[lastStackBuilderBlock - 2] = true;

                    // Now delete block boundary created by dup_copy.
                    boundaries.RemoveAt(lastStackBuilderBlock - 1);
                    isMergedBlock.RemoveAt(lastStackBuilderBlock - 1);

                    // Fix the indexes.
                    lastStackBuilderBlock--;
                    lastUnmergableBoundary--;
                }
            }
            ...
            for (int iBoundary = lastStackBuilderBlock - 1; iBoundary >= 1; iBoundary--)
            {
                // Check if this block really puts anything on the stackLevel.
                //
                IlInstruction blockBeginInstruction = block.Children[boundaries[iBoundary - 1]].GetInstructionAt(0);
                IlInstruction blockEndInstruction = block.Children[boundaries[iBoundary] - 1].GetInstructionAt(0);
                bool skipDup = false;
                ...
                {
                    // mark Dup block as merged block.
                    isMergedBlock[iBoundary - 2] = true;
                    ...
                    blockBeginInstruction = block.Children[boundaries[iBoundary - 2]].GetInstructionAt(0);
                }
                ...
                //
                bool isStackBuilder =
                    blockBeginInstruction.StackLevelBefore < blockEndInstruction.StackLevelAfter;
                ...
                if (isStackBuilder &&
                    (!containsConsumerBlock ||
                     (lastStackBuilderBlock < lastUnmergableBoundary ||
                      blockEndInstruction.StackLevelAfter < block.Children[boundaries[lastStackBuilderBlock]].StackBefore) ||
                      (lastStackBuilderBlock == lastUnmergableBoundary &&
                      blockEndInstruction.StackLevelAfter > block.Children[boundaries[lastStackBuilderBlock] - 1].StackBefore)))
                {
                    ...
                    if (iBoundary < boundaries.Count - 3 &&
                        block.Children[boundaries[iBoundary + 2] - 1].StackAfter == blockEndInstruction.StackLevelAfter)
                    {
                        boundaries.RemoveAt(iBoundary + 1);
                        isMergedBlock.RemoveAt(iBoundary + 1);
                        isMergedBlock[iBoundary] = block.Parent != null;
        ...
        /// <summary>
        /// Merges the corss referencing blocks.
        /// </summary>
        /// <param name="block">The block.</param>
        /// <param name="boundaries">The boundaries.</param>
        /// <param name="isMergedBlock">The is merged block.</param>
