...
namespace NScript.CLR.Decompiler.Blocks
{
    ...
    {
        /// <summary>
        /// Processes the breaks.
        /// </summary>
        /// <param name="block">The block.</param>
        public static void ProcessBreaks(Block block)
        {
            JumpBlockBuilder.Process(block, true, false);
        }

        /// <summary>
        /// Processes the continues.
        /// </summary>
        /// <param name="block">The block.</param>
        public static void ProcessContinues(Block block)
        {
            JumpBlockBuilder.Process(block, false, false);
        }

        /// <summary>
        /// Processes the remaining gotos.
        /// </summary>
        /// <param name="block">The block.</param>
        public static void ProcessRemainingGotos(Block block)
        {
            JumpBlockBuilder.Process(
                block,
                false,
                true);
        }
            ...
            bool processGotos)
        {
            ...
            for (int i = 0; i < block.Children.Count; i++)
            {
                ...
                DoWhileBlock doWhileBlock = block as DoWhileBlock;
                ...
                if (switchBlock != null)
                {
                    foreach (var kvPair in switchBlock.CaseBlocks)
                    {
                        JumpBlockBuilder.Process(
                            kvPair.Value,
                            processBreaks,
            ...
            if ((basicBlock is StackedBlock && lastInstruction.FlowType == FlowType.ConditionalBranch)
                || (basicBlock is InstructionBlock
                    && (lastInstruction.FlowType == FlowType.Branch
                        || lastInstruction.FlowType == FlowType.Leave)))
            {
                JumpBlockBuilder.Create(basicBlock, processBreaks, processGotos);
            }
        }
